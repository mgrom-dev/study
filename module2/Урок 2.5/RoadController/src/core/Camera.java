...
public class Camera //добавляем класс камера
{
    public static Car getNextCar() //метод генерации новой машины
    {
        ...
        car.number = randomNumber; //устанавливаем номер машины
        car.height = randomHeight; //устанавливаем высоту машины
        car.weight = randomWeight; //устанавливаем вес машины
        car.hasVehicle = Math.random() > 0.5; //булевая переменная, которая озночает что машина с прицепом (вероятность 50%)
        car.isSpecial = Math.random() < 0.15; //булевая переменная, которая озночает что это спецтранспорт (вероятность 15%)
